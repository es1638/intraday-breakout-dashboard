...
def passes_screening(ticker):
    try:
        ...
        if hist.empty or "Close" not in hist.columns or "Volume" not in hist.columns:
            st.info(f"{ticker}: Missing Close or Volume columns")
            return False

        avg_volume = hist["Volume"].tail(30).mean()
        if avg_volume < 10_000_000:
            st.info(f"{ticker}: Avg volume too low ({avg_volume})")
            return False

        high_52w = hist["High"].rolling(window=252).max().iloc[-1]
        current_price = hist["Close"].iloc[-1]
        if float(current_price) < 0.6 * float(high_52w):
            st.info(f"{ticker}: Price too far from 52w high ({current_price} vs {high_52w})")
            return False
